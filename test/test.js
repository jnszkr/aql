...
describe("items", function() {
  describe("#find()", function() {
    ...
    it("should present criterias when it is set in parameter", function() {
      ...
      assert.equal(
        aql.items.find(criteria).query,
        'items.find({"repo":"abc"})'
      );
      assert.equal(
        aql.builds.find(criteria).query,
        'builds.find({"repo":"abc"})'
      );
      assert.equal(
  ...
  describe("#query()", () => {
    ...
    it("should create a new DomainQuery instance for each query", () => {
      assert.equal(
        aql.items.find().include("name").query,
        'items.find().include("name")'
      );
      assert.equal(
        aql.items.find().include("repo").query,
        'items.find().include("repo")'
      );
    });
  });
});
